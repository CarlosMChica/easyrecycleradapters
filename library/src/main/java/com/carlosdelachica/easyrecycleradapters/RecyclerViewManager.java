...
public class RecyclerViewManager<V, VH extends CommonViewHolder<V>> implements CommonViewHolder.OnItemClickListener,
    ...
    private void initAuxTextView() {
        updateAuxTextView(EmptyListTextViewState.LOADING);
    }
    ...
    public void add(List<V> data) {
        adapter.add(data);
        updateAuxTextView(data.size() > 0 ? EmptyListTextViewState.HIDDEN : EmptyListTextViewState.EMPTY);
    }

    public void add(V data, int position) {
        adapter.add(data, position);
        updateAuxTextView(EmptyListTextViewState.HIDDEN);
    }
    ...
    public void remove(V data) {
        adapter.remove(data);
        updateAuxTextView(adapter.getItemCount() > 0 ? EmptyListTextViewState.HIDDEN : EmptyListTextViewState.EMPTY);
    }
    ...
    private void updateAuxTextView(EmptyListTextViewState state) {
        switch (state) {
            case HIDDEN:
